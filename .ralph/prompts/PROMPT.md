...
1. Run: `cd src && npm run lint -- --fix`
2. Run: `cd src && npx tsc --noEmit` (typecheck)
3. Run: `cd src && npm test`
...
2. **Run**: `cd src && npm audit` (check for vulnerable dependencies)
...
2. `git -C src add -A`
3. `git -C src commit -m "type(scope): description"`
4. `git -C src push origin feat/$FEATURE`
